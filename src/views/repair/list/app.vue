        ...
        <scroll  class="main" :on-reach-bottom="handleReachBottom">
                <Card dis-hover v-for="(item, index) in list" class="list" :key="index">
                    <div  @click="toDetail(item.deviceRepairPk,item.devicePk)" >
                        <p>
                            <span>设备编号：</span>{{item.deviceCd}}
                        </p>
                        <p>
                            <span>小区名称：</span>{{item.estateNm}}
                        </p>
                        <p>
                            <span>设备名称：</span>{{item.deviceScatNm}}
                        </p>
                        <p>
                            <span>维修单号：</span>{{item.billCode}}
                        </p>
                        <p>
                            <span>申请时间：</span>{{item.applicationTm | toDate}}
                        </p>
                        <p>
                            <span>申请单位：</span>{{item.applicantUnitNm}}
                        </p>
                        <p>
                            <span>申请人：{{item.proposer}}</span>
                        </p>
                        <p>
                            <span>处理状态：</span>{{item.statNm}}
                        </p>
        ...
        methods: {
            getList(){
                ...
                let $q = new Promise((resolve,reject)=>{
                    ...
                    this.until.get('/ph/deviceRepair/page',param)
                        .then(res=>{
                            if(res.status == 200){
                                this.spinShow = false
