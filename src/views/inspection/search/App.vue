    ...
    <Form :label-width="80" class="search">
        ...
        <Row>
          <Col span="11">
            <DatePicker
              type="date"
              placeholder
              v-model="search.inspectPlanTimeBeg"
          ...
          <Col span="11">
            <DatePicker
              type="date"
              placeholder
              v-model="search.inspectPlanTimeEnd"
      ...
      <FormItem label="供水模式">
        <el-select v-model="search.waterSupplyModeCd" filterable placeholder="供水模式" clearable>
          <el-option
            v-for="item in waterSupplyModeNms"
            :key="item.cd"
            :label="item.nm"
            :value="item.cd"
          ></el-option>
  ...
  methods: {
    toSearch() {
      ...
      if (this.search.usageDate2) {
        ...
        this.search.usageDate2 =
          myDate2.year + "-" + myDate2.month + "-" + myDate2.day;
      }

      window.location.href =
        this.type + ".html?search=" + JSON.stringify(this.search);
    },
    ...
    getSelect() {
      ...
      //供水模式
      this.until.get("/general/cat/listByPrntCd?prntCd=6a").then(res => {
