...
<template>
    <div id="container">
        ...
        <div class="menu">
            <p @click="change(1)" :class="{active:type==1}">工单指派</p>
            <p @click="change(2)" :class="{active:type==2}">工单确认</p>
            ...
            <scroll  class="main" :on-reach-bottom="handleReachBottom">
                <Card dis-hover v-for="(item, index) in list" class="list" :key="index" >
                    <div @click="toDetail(item.dispatchSendPk)">
                        <p>
                            <span>工单来源：</span>{{item.dispatchFromNm}}
                        </p>
                        <p>
                            <span>部门名称：</span>{{item.bmNm}}
                        </p>
                        <p>
                            <span>泵房名称：</span>{{item.address}}
                        </p>
                        <p>
                            <span>处理状态：</span>{{item.dealStatus | state}}
                        </p>
                        <p>
                            <span>故障现象：</span>{{item.proLvNm}}
                        </p>
                        <p>
                            <span>派单时间：</span>{{item.sendTm}}
                        </p>
                        <img src="../components/img/toDetail.png"/>
                        <span v-if="type==2" class="btn" @click.stop="toEdit(item.dispatchSendPk,'confirm')">确认</span>
                        <span v-if="type==3 && item.dealStatus==5" class="btn" @click.stop="toEdit(item.dispatchSendPk,'visit')">回访</span>
                    </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.type = this.until.getQueryString('type')?this.until.getQueryString('type'):1
            this.getList()
        },
        methods: {
            getList(){
