            ...
            <scroll  class="main" :on-reach-bottom="handleReachBottom">
                <Card dis-hover v-for="(item, index) in list" class="list" :key="index" >
                    <div @click="toDetail(item.dispatchSendPk)">
                        <p>
                            <span>水箱编号：</span>{{item.dispatchFromNm}}
                        </p>

                        <p>
                            <span>清洗日期：</span>{{item.address}}
                        </p>
                        <p>
                            <span>小区名称：</span>{{item.bmNm}}
                        </p>
                        <p>
                            <span>容积（吨）：</span>{{item.dealStatus | state}}
                        </p>
                        <p>
                            <span>清理容积：</span>{{item.proLvNm}}
                        </p>
                        <p>
                            <span>星期：</span>{{item.sendTm}}
                        </p>
                        <p>
                            <span>行政区域：</span>{{item.sendTm}}
                        </p>
